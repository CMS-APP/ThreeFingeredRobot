...
if (simConfig.obj == 1) || (simConfig.obj == 2) || (simConfig.obj == 3)
    ...
    [poc1, dpoc1, poc2, dpoc2, poc3, dpoc3, pc1, pc2, ... ,
        pc3] = calcContactPntPos(pt1, pt2, pt3, nx1, nx2, ny3, dpt1, dpt2, dpt3, po, wo, podot, r);
...
Phi3 = ny3' * pt3 + ny3' * r * ny3 - ny3' * po - ny3' * poc3;        % 1x1
...
u1T =  -fd * Jv1' * ((pt1 - middle) / norm(pt1 - middle));
u2T =  -fd * Jv2' * ((pt2 - middle) / norm(pt2 - middle));
u3T =  -fd * Jv3' * ((pt3 - middle) / norm(pt3 - middle));
...
theta1 = calcHorizontalAngle(f1Base, middle, pt1);
theta2 = calcHorizontalAngle(f2Base, middle, pt2);
theta3 = calcHorizontalAngle(f3Base, middle, pt3);

zeta1 = calculateVerticalAngle(pt1, pc1, middle);
zeta2 = calculateVerticalAngle(pt2, pc2, middle);
zeta3 = calculateVerticalAngle(pt3, pc3, middle);

u1R = r * -fd * Jw1' * (-nz1 * sin(theta1) + ny1 * sin(zeta1));
u2R = r * -fd * Jw2' * (-nz2 * sin(theta2) + -ny2 * sin(zeta2));
u3R = r * -fd * Jw3' * (nz3 * sin(theta3) + -ny3 * sin(zeta3));
...
function [poc1, dpoc1, poc2, dpoc2, poc3, dpoc3, pc1, pc2, pc3] = calcContactPntPos(pt1, pt2, pt3, nx1, nx2, ny3, dpt1, dpt2, dpt3, po, wo, podot, r)
    ...
    poc3  = pt3 + r * ny3 - po;
    dpoc3 = dpt3 - r * skew(ny3) * wo - podot;
...
function theta = calcHorizontalAngle(fBase, middle, pt)
    fBaseX = fBase(1);
    fBaseY = fBase(2);
    ...
    theta = pointAngle - middleAngle;
    return 
end

function zeta = calculateVerticalAngle(pt, pc, middle) 
    ptX = pt(1);
    ptY = pt(2);
    ptZ = pt(3);
    ...
    horizontalTValue = sqrt(deltaTX^2 + deltaTY^2);
    horizontalCValue = sqrt(deltaCX^2 + deltaCY^2);
    
    tipAngle = atan2(horizontalTValue, deltaTZ);
    contactAngle = atan2(horizontalCValue, deltaCZ);
    
    zeta = tipAngle - contactAngle;
